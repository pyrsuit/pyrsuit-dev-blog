...
**[\`threading.Lock\`](https://docs.python.org/3/library/threading.html#threading.Lock)** - protects the \`_active\` counter that tracks how many threads are currently inside \`do_work\`. Since multiple OS threads touch that counter simultaneously, a plain asyncio lock won't do - it has to be a threading primitive that works at the OS level.
...
- The \`_active\` counter and \`assert\` are there to **verify** the semaphore is actually working - if more than \`CONCURRENCY\` threads ever enter \`do_work\` simultaneously, the assert fires.
...
*The test numbers will vary by machine.*
...
*The full script is available on [GitHub](https://github.com/pyrsuit/scripts/blob/main/gil-benchmark/main.py).*
`;export{e as default};
